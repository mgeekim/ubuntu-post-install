...
function install_zsh() {
    header "ZSHELL"
    myapt install zsh
    mycmd chsh -s `which zsh` # Does not working for WSL. WSL has own settings.
    mycmd sh -c "$(wget -O- https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh)"
    mycmd git clone https://github.com/bhilburn/powerlevel9k.git ~/.oh-my-zsh/custom/themes/powerlevel9k
    mycmd ln -s "$(pwd)"/asset/zshell/.zshrc ~/.zshrc
}
